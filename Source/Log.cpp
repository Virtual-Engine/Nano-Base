...
#define CHARS_FOR_PARAMS 3500
	...
	void Log::Init(HMODULE hModule) {
		...
		if (!AttachConsole(GetCurrentProcessId()))
			AllocConsole();
		SetConsoleTitleW(MODBASE_WIDE(Name));
	...
	void Log::Debug(const char* fmt, ...)
	{
		...
		sprintf_s(szTimestamp, "[%s] [%02d:%02d:%02d] DEBUG: %%s", Name, current_tm.tm_hour, current_tm.tm_min, current_tm.tm_sec);
	...
	void Log::Msg(const char* fmt, ...) {
		va_list va_alist;
		va_start(va_alist, fmt);
		Log::Main_Logger(Name, fmt, va_alist);
		va_end(va_alist);
	}
	...
	void Log::Error(const char* fmt, ...) {
		...
		va_start(va_alist, fmt);
		Log::Main_Logger(Name, _strdup(Error_msg.c_str()), va_alist);
		_vsnprintf_s(chParameters, sizeof(chParameters), fmt, va_alist);
		va_end(va_alist);
		sprintf_s(chLogBuff, szTimestamp, chParameters);
	...
	void Log::Fatal(const char* fmt, ...) {
		...
		localtime_s(&current_tm, &current_time);
		sprintf_s(szTimestamp, "[%s] [%02d:%02d:%02d] FATAL: %%s\n", Name, current_tm.tm_hour, current_tm.tm_min, current_tm.tm_sec);
		va_start(va_alist, fmt);
		string Error_msg = string("FATAL ERROR: ") + string(fmt);
		Log::Main_Logger(Name, _strdup(Error_msg.c_str()), va_alist);
		_vsnprintf_s(chParameters, sizeof(chParameters), fmt, va_alist);
	...
	void Log::Main_Logger(const char* type, const char* format, va_list args)
	{
